...
class CPT {
	...
	/**
	 * Holds the partitive name of the post type. This is a human friendly
	 * name, capitalized with spaces assigned on __construct().
	 *
	 * @var string $partitive Singular post type name.
	 */
	public $partitive;

	/**
	 * Holds the partitive-plural name of the post type. This is a human friendly
	 * name, capitalized with spaces assigned on __construct().
	 *
	 * @var string $partitive_plura Singular post type name.
	 */
	public $partitive_plural;
	...
	function __construct( $post_type_names, $options = [ ] ) {
		...
		if ( is_array( $post_type_names ) ) {
			...
			// Set the partitive name label.
			$this->partitive = $this->get_partitive();

			// Set the partitive-plural name label.
			$this->partitive_plural = $this->get_partitive_plural();
	 ...
	 * @param  string $function      Function to hook that will run on @action.
	 * @param  int    $priority      Order in which to execute the function, relation to other function hooked to this
	 *                               action.
	 ...
	 * Get partitive
	 *
	 * Returns the friendly partitive name.
	 *
	 *    ucwords      capitalize words
	 *    strtolower   makes string lowercase before capitalizing
	 *    str_replace  replace all instances of _ to space
	 *
	 * @param string $name The name name you want to be partitive.
	 *
	 * @return string The friendly partitive name.
	 */
	function get_partitive( $name = null ) {
	...
	/**
	 * Get partitive_plural
	 *
	 * Returns the friendly partitive_plural name.
	 *
	 *    ucwords      capitalize words
	 *    strtolower   makes string lowercase before capitalizing
	 *    str_replace  replace all instances of _ to space
	 *
	 * @param string $name The name name you want to be partitive plural.
	 *
	 * @return string The friendly partitive name.
	 */
	function get_partitive_plural( $name = null ) {
		...
		// Return the partitive plural name.
		return $this->get_human_friendly( $name );
	}
	...
	function register_post_type() {
		...
		$partitive        = $this->partitive;
		$partitive_plural = $this->partitive_plural;
		$slug             = $this->slug;

		// Default labels.
		$labels = [
			'name'               => sprintf( '%s', _x( $plural, 'Plural', $this->textdomain ) ),
			'singular_name'      => sprintf( '%s', _x( $singular, 'Single', $this->textdomain ) ),
			'menu_name'          => sprintf( '%s', _x( $plural, 'Plural', $this->textdomain ) ),
			'all_items'          => sprintf( '%s', _x( $plural, 'Plural', $this->textdomain ) ),
			'add_new'            => __( 'Add New', $this->textdomain ),
			'add_new_item'       => sprintf( __( 'Add New %s', $this->textdomain ),
				_x( $singular, 'Single', $this->textdomain ) ),
			'edit_item'          => sprintf( __( 'Edit %s', $this->textdomain ),
				_x( $partitive, 'Partitive', $this->textdomain ) ),
			...
			'search_items'       => sprintf( __( 'Search %s', $this->textdomain ),
				_x( $plural, 'Plural', $this->textdomain ) ),
			'not_found'          => sprintf( __( 'No %s found', $this->textdomain ),
				_x( $partitive_plural, 'Partitive plural', $this->textdomain ) ),
			'not_found_in_trash' => sprintf( __( 'No %s found in Trash', $this->textdomain ),
				_x( $partitive_plural, 'Partitive plural', $this->textdomain ) ),
			'parent_item_colon'  => sprintf( __( 'Parent %s:', $this->textdomain ),
				_x( $singular, 'Single', $this->textdomain ) ),
			'archives'           => sprintf( '%s', _x( $plural, 'Plural', $this->textdomain ) )
		];

		// Default options.
		$defaults = [
			'label'   => sprintf( '%s', _x( $plural, 'Plural', $this->textdomain ) ),
			'labels'  => $labels,
			'public'  => true,
			'rewrite' => [
				'slug' => $slug,
	...
	function register_taxonomy( $taxonomy_names, $options = [ ] ) {
		...
		if ( is_array( $taxonomy_names ) ) {
			...
			$partitive        = $this->get_partitive( $taxonomy_name );
			$partitive_plural = $this->get_partitive_plural( $taxonomy_name );
		...
		$labels = [
			'name'                       => sprintf( '%s', _x( $plural, 'Plural', $this->textdomain ) ),
			'singular_name'              => sprintf( '%s', _x( $singular, 'Singular', $this->textdomain ) ),
			'menu_name'                  => sprintf( '%s', _x( $plural, 'Plural', $this->textdomain ) ),
			'all_items'                  => sprintf( __( 'All %s', $this->textdomain ),
				_x( $plural, 'Plural', $this->textdomain ) ),
			'edit_item'                  => sprintf( __( 'Edit %s', $this->textdomain ),
				_x( $partitive, 'Partitive', $this->textdomain ) ),
			'view_item'                  => sprintf( __( 'View %s', $this->textdomain ),
				_x( $partitive, 'Partitive', $this->textdomain ) ),
			'update_item'                => sprintf( __( 'Update %s', $this->textdomain ),
				_x( $singular, 'Single', $this->textdomain ) ),
			'add_new_item'               => sprintf( __( 'Add New %s', $this->textdomain ),
				_x( $singular, 'Single', $this->textdomain ) ),
			'new_item_name'              => sprintf( __( 'New %s Name', $this->textdomain ),
				_x( $singular, 'Single', $this->textdomain ) ),
			'parent_item'                => sprintf( __( 'Parent %s', $this->textdomain ),
				_x( $singular, 'Single', $this->textdomain ) ),
			'parent_item_colon'          => sprintf( __( 'Parent %s:', $this->textdomain ),
				_x( $singular, 'Single', $this->textdomain ) ),
			'search_items'               => sprintf( __( 'Search %s', $this->textdomain ),
				_x( $partitive_plural, 'Partitive plural', $this->textdomain ) ),
			'popular_items'              => sprintf( __( 'Popular %s', $this->textdomain ),
				_x( $plural, 'Plural', $this->textdomain ) ),
			'separate_items_with_commas' => sprintf( __( 'Separate %s with commas', $this->textdomain ),
				_x( $plural, 'Plural', $this->textdomain ) ),
			'add_or_remove_items'        => sprintf( __( 'Add or remove %s', $this->textdomain ),
				_x( $partitive_plural, 'Partitive plural', $this->textdomain ) ),
			'choose_from_most_used'      => sprintf( __( 'Choose from most used %s', $this->textdomain ),
				_x( $plural, 'Plural', $this->textdomain ) ),
			'not_found'                  => sprintf( __( 'No %s found', $this->textdomain ),
				_x( $partitive_plural, 'Partitive plural', $this->textdomain ) ),
		];
