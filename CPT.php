...
class CPT {
	...
	*/
	...
	function add_admin_columns($columns) {
		...
		if(!isset($this->columns)) {
			...
			/* 
				Others
			$colums['icon'] = null, // post_thumbnail (CPT populated)
			$columns['author'] = __('Author') // author (WP populated)
			$columns['comments'] = '<img alt="Comments" src="'. site_url() .'/wp-admin/images/comment-grey-bubble.png">'; // number of comments (wp populated)
			$colums['post_id'] = _('ID') // post id ()
			*/
	...
	*/
	...
	function populate_admin_columns($column, $post_id) {
		...
		switch($column) {
			...
			//case ($this->is_meta_key($column)) : 

				// print_r(get_post_meta($post->ID, $column));

			// break;
			// case (preg_match('/^post_/', $column) ? true : false) :
			// turns slug post_post_title into $post->post_title
			// 	$x = substr($column, 5);
			// 	print_r($post);
			// 	echo $post->$x;
    ...
    */
    function sortable($columns = array()) {
    	...
    	/*
			array(
				'column' => array('meta_key', is_integer = default false)
			)
    ...
    // used by filter in sortable, merges custom sortable columns with wordpress default
